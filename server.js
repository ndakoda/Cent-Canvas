...
async function verifyPayment(details) {
  ...
  if (railRates[details.rail].chain === "evm") {
    return details.txHash ? verifyEvmPayment(details) : findRecentEvmPayment(details);
  }

  return details.txHash ? verifySolanaPayment(details) : findRecentSolanaPayment(details);
}

async function findRecentEvmPayment({ rail, from, to, cents }) {
  const expectedWei = weiForCents(cents, rail);
  const rpcUrl = railRates[rail].rpcUrl;
  const expectedFrom = normalizeEvmAddress(from);
  const expectedTo = normalizeEvmAddress(to);
  const latestHex = await rpcRequest(rpcUrl, { jsonrpc: "2.0", id: 1, method: "eth_blockNumber", params: [] });
  const latest = Number.parseInt(latestHex, 16);
  const claims = loadClaims();

  for (let block = latest; block >= Math.max(0, latest - 360); block -= 1) {
    ...
    const match = (payload?.transactions || []).find((tx) => {
      if (txHashAlreadyUsed(claims, tx.hash)) return false;
      if (normalizeEvmAddress(tx.from) !== expectedFrom) return false;
      if (normalizeEvmAddress(tx.to) !== expectedTo) return false;
      return BigInt(tx.value || "0x0") >= expectedWei;
    });

    if (!match) continue;

    const receipt = await rpcRequest(rpcUrl, {
      jsonrpc: "2.0",
      id: 1,
      method: "eth_getTransactionReceipt",
      params: [match.hash],
    });
    if (receipt?.status === "0x1") return match.hash;
  }

  throw new Error("No matching confirmed payment found yet. Send funds to the destination, wait a moment, then click verify again.");
}

async function findRecentSolanaPayment({ rail, from, to, cents }) {
  const expectedLamports = lamportsForCents(cents, rail);
  const rpcUrl = railRates[rail].rpcUrl;
  const claims = loadClaims();
  const signatures = await rpcRequest(rpcUrl, {
    jsonrpc: "2.0",
    id: 1,
    method: "getSignaturesForAddress",
    params: [to, { limit: 75, commitment: "confirmed" }],
  });

  for (const item of signatures || []) {
    const signature = item.signature;
    if (!signature || item.err || txHashAlreadyUsed(claims, signature)) continue;
    ...
    if (paidLamports >= expectedLamports) return signature;
  }

  throw new Error("No matching confirmed Solana payment found yet. Send funds to the destination, wait a moment, then click verify again.");
}
...
function attachVerifiedPayment(prepared, txHash) {
  if (!txHash) return;

  prepared.next.pixels = prepared.next.pixels.map(([key, pixel]) => [
    key,
    pixel?.cents > 0 && !pixel.txHash ? { ...pixel, txHash } : pixel,
  ]);

  prepared.next.activity = prepared.next.activity.map((item) => (
    item?.cents > 0 && !item.txHash ? { ...item, txHash } : item
  ));
}
...
async function handleApi(req, res, url) {
  ...
  try {
    ...
    if (req.method === "POST" && url.pathname === "/api/claims") {
      ...
      const txHash = await verifyPayment(prepared.verification);
      attachVerifiedPayment(prepared, txHash);
